//! Token store + mint/verify (SPEC-005; EP-006 M1, shared M3).
//!
...
//!
//! Pepper comes from `VIHS_TOKEN_PEPPER` and MUST be identical across every
//! service that verifies tokens (orchestrator + memoryd) — a per-process
//! ephemeral pepper breaks cross-service verification (EP-006 M3 Decision
//! Log). Stage/prod MUST set it or the service refuses to start (STOP S1);
//! dev falls back to an ephemeral pepper with a loud log, but memoryd
//! requires it because it verifies tokens minted by another process.
...
/// User/admin token lifetime (SPEC-005: user tokens are long-lived until
/// revoked — mint passes an explicit TTL).
pub const DEFAULT_TOKEN_TTL: Duration = Duration::from_secs(24 * 60 * 60);

/// Pod token lifetime (SPEC-005 A3: minted per assignment, TTL ≤15 min,
/// renewed by the orchestrator while the assignment holds).
pub const POD_TOKEN_TTL: Duration = Duration::from_secs(15 * 60);
...
/// The three principals (SPEC-005 terms/model).
...
impl TokenStore {
    ...
    pub async fn seed(
        ...
        let raw = URL_SAFE_NO_PAD
            .decode(token)
            .map_err(|_| TokenError::Authz("seed token must be base64url".into()))?;
        if raw.len() != TOKEN_TOTAL_BYTES {
            return Err(TokenError::Authz(
    ...
    pub async fn verify(&self, token: &str) -> Result<Principal, TokenError> {
        let raw = URL_SAFE_NO_PAD
            .decode(token)
            .map_err(|_| TokenError::Authz("malformed token".into()))?;
        if raw.len() != TOKEN_TOTAL_BYTES {
            return Err(TokenError::Authz("malformed token".into()));
        }
        let (id_bytes, _) = raw.split_at(TOKEN_ID_BYTES);
        let token_id = URL_SAFE_NO_PAD.encode(id_bytes);
        let key = redis_key(&token_id);
        ...
        let Some(owner) = rec.get("owner") else {
            return Err(TokenError::Authz("unknown token".into()));
        };
        ...
        let exp: i64 = rec
            ...
            .ok_or_else(|| TokenError::Authz("corrupt token record".into()))?;
        let revoked: i64 = rec.get("revoked").and_then(|v| v.parse().ok()).unwrap_or(0);
        if revoked != 0 {
            return Err(TokenError::Authz("token revoked".into()));
        }
        if chrono::Utc::now().timestamp() > exp {
            return Err(TokenError::Authz("token expired".into()));
        }
    ...
    fn hash(&self, token: &str) -> Result<String, TokenError> {
        ...
        let argon = Argon2::new(
            ...
            argon2::Params::new(ARGON2_M_COST, ARGON2_T_COST, ARGON2_P_COST, None)
                .map_err(|e| TokenError::Upstream(format!("argon2 params: {e}")))?,
        );
        argon
            .hash_password(salted.as_bytes(), &salt)
            .map(|h| h.to_string())
            .map_err(|e| TokenError::Upstream(format!("argon2 hash: {e}")))
    }

    fn verify_hash(&self, phash: &str, token: &str) -> Result<(), TokenError> {
        let salted = format!("{token}{}", self.pepper);
        let parsed = PasswordHash::new(phash)
            .map_err(|_| TokenError::Authz("corrupt token record".into()))?;
        let argon = Argon2::new(
            argon2::Algorithm::Argon2id,
            argon2::Version::V0x13,
            argon2::Params::new(ARGON2_M_COST, ARGON2_T_COST, ARGON2_P_COST, None)
        ...
        match argon.verify_password(salted.as_bytes(), &parsed) {
            Ok(()) => Ok(()),
            Err(_) => Err(TokenError::Authz("token mismatch".into())),
        }
    }
}
...
mod tests {
    ...
    async fn wrong_token_rejected() {
        ...
        assert!(matches!(err, TokenError::Authz(_)));
    }
    ...
    #[tokio::test]
    async fn cross_store_verify_requires_same_pepper() {
        // M3 regression: two stores with DIFFERENT peppers must not verify
        // each other's tokens — the shared-pepper contract (VIHS_TOKEN_PEPPER)
        // is what makes orchestrator↔memoryd verification work.
        let a = TokenStore::connect("redis://127.0.0.1:6379", "pepper-a".to_string())
            .await
            .unwrap();
        let b = TokenStore::connect("redis://127.0.0.1:6379", "pepper-b".to_string())
            .await
            .unwrap();
        let token = a
            .mint("owner-1", Scope::User, DEFAULT_TOKEN_TTL)
            .await
            .unwrap();
        let err = b.verify(&token).await.unwrap_err();
        assert!(matches!(err, TokenError::Authz(_)));
    }
}
