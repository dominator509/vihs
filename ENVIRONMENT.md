...
| VIHS_TTS_THREADS | no | stage,prod | 2 | N | bounded intra-op thread pool for the in-process PiperVoice ONNX session (EP-010 M2 contention fix; onnxruntime default 0 = all cores thrashes llama-server on the same vCPUs) | int ≥ 1, 0 = all cores |
