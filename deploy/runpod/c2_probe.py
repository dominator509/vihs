#!/usr/bin/env python3
"""Real-conversation probe: run build_stages(real=True) + Conversation on a pod.

Exercises the EXACT conversation wrapper (PlaybackMonitor, Metrics,
_build_prompt via memoryd, append_buffer) that the smoke uses, with
traced LLM/TTS stages so a hang pinpoints the stage. WebRTC is bypassed
(call _handle_turn directly) to isolate the turn path from ICE.
...
report("C2_WHEELS: " + ("OK" if r.returncode == 0 else "FAIL " + r.stderr[-300:]))
...
async def _main():
    ...
    report("C2_START model_dir=" + model_dir + " memoryd=" + memoryd)

    stages, monitor = build_stages([], real=True)
    report("C2_STAGES_BUILT")

    # Trace the real stage adapters so a hang pinpoints the stage.
    orig_llm_stream = stages.llm.stream
    orig_tts_stream = stages.tts.stream

    async def traced_llm(prompt):
        t0 = time.monotonic()
        report("C2_LLM_BEGIN prompt_len=%d" % len(prompt))
        n = 0
        async for c in orig_llm_stream(prompt):
            n += 1
            if n == 1:
                report("C2_LLM_FIRST_DELTA %.1fs" % (time.monotonic() - t0))
            yield c
        report("C2_LLM_END deltas=%d %.1fs" % (n, time.monotonic() - t0))

    async def traced_tts(clause, voice):
        t0 = time.monotonic()
        report("C2_TTS_BEGIN clause=%r" % clause[:30])
        n = 0
        async for ch in orig_tts_stream(clause, voice):
            n += 1
            if n == 1:
                report("C2_TTS_FIRST_CHUNK %.1fs" % (time.monotonic() - t0))
            yield ch
        report("C2_TTS_END chunks=%d %.1fs" % (n, time.monotonic() - t0))

    stages.llm.stream = traced_llm
    stages.tts.stream = traced_tts

    session_id = "c2-" + uuid.uuid4().hex[:12]
    ...
    report("C2_CONVO_BUILT")

    t0 = time.monotonic()
    try:
        await asyncio.wait_for(convo._handle_turn("First message before disconnect."), timeout=150.0)
        report("C2_TURN_DONE %.1fs" % (time.monotonic() - t0))
    except asyncio.TimeoutError:
        report("C2_TURN_HUNG >150s")
    except Exception as e:
        import traceback
        report("C2_TURN_ERROR %r %s" % (e, traceback.format_exc()[-600:]))
    finally:
        try:
            await convo.stop()
        except Exception:
...
def check_reports() -> tuple[bool, str]:
    ...
    return ("C2_DONE" in data), data[-3000:]
